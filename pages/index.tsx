...
export default function Home() {
  ...
  const handleAnswer = async () => {
    ...
    const context = results?.map((d: any) => d.content).join("\n\n");
  ...
  const handleSave = () => {
    localStorage.setItem("IK_MATCH_COUNT", matchCount.toString());
    localStorage.setItem("IK_MODE", mode);
  ...
  const handleClear = () => {
    localStorage.removeItem("IK_MATCH_COUNT");
    localStorage.removeItem("IK_MODE");
  ...
  useEffect(() => {
    const IK_MATCH_COUNT = localStorage.getItem("IK_MATCH_COUNT");
    const IK_MODE = localStorage.getItem("IK_MODE");

    if (IK_MATCH_COUNT) {
      setMatchCount(parseInt(IK_MATCH_COUNT));
    }

    if (IK_MODE) {
      setMode(IK_MODE as "search" | "chat");
    }
  ...
  return (
    <>
      <Head>
        <title>Imran Khan GPT</title>
        <meta
          name="description"
          content="AI-powered search and chat for Imran Khan's speeches, tweets, and interviews."
        />
        <meta
          name="viewport"
          ...
          <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center px-3 pt-4 sm:pt-8">
            ...
            <div className="relative w-full mt-4">
              ...
              <input
                ...
                type="text"
                placeholder="What is Imran Khan's vision for Pakistan?"
                value={query}
            ...
            {loading ? (
                        ...
                        <div className="flex justify-between">
                          <div>
                            ...
                            <div className="mt-1 font-bold text-sm">
                              {chunk.content_date} • {chunk.content_source}
                            </div>
                          </div>
                          <a
              ...
              <div className="mt-6 text-center text-lg">
                AI-powered search & chat for Imran Khan's speeches, tweets, and interviews.
              </div>
            )}
          </div>
        </div>
